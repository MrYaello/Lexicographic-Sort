...
public class Arreglos {
    ...
    quickSort(T[] a, int st, int e, Comparator<T> c) {
      if (e <= st) return;
      int i = st + 1;
      int j = e;

      while (i < j) {
        if (c.compare(a[j], a[st]) <= 0 && c.compare(a[i], a[st]) > 0) intercambia(a, i++, j--);
        else if (c.compare(a[i], a[st]) <= 0) i++; 
        else j--;
      }

      if (c.compare(a[i], a[st]) > 0) i--;
      intercambia(a, st, i);
      quickSort(a, st, i - 1, c);
      quickSort(a, i + 1, e, c);
    }
